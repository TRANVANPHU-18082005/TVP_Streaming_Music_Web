...
import { AlertCircle, ListMusic, ChevronRight } from "lucide-react";
  ...
  <div className="flex items-start justify-between gap-4 mb-7 sm:mb-8">
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-overline" style={{ color: "hsl(var(--wave-5))" }}>
          Libraries
        </span>
      </div>
    ...
    <Link
      ...
      className={cn(
        ...
        "text-sm font-medium text-wave-5 opacity-70",
        "hover:text-wave-5 transition-colors duration-200 hover:opacity-100",
      )}
...
// ─────────────────────────────────────────────────────────────────────────────
export function FavouritePlaylist() {
  const { data: dashboard, isLoading, isError } = useProfileDashboard();
  const playlists = dashboard?.library.playlists;
  ...
  const renderContent = () => {
    ...
    if (isError)
      return <ErrorState message="Không thể tải danh sách playlist." />;
    if (!playlists?.length)
      return <EmptyState message="Chưa có playlist nổi bật nào được thêm." />;
