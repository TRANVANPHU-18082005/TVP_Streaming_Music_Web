...
const ArtworkCard = memo(
  ({
    ...
    const boxShadow = useTransform(
      [shadowX, shadowY],
      ([sx, sy]) =>
        `${sx} ${sy} 48px hsl(${artist.themeColor} / 0.35), 0 24px 64px rgba(0,0,0,0.45)`,
    );
      ...
      <AnimatePresence mode="popLayout" custom={direction} initial={false}>
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background: `radial-gradient(circle, hsl(${artist.themeColor} / 0.7) 0%, transparent 70%)`,
              filter: "blur(55px)",
          ...
          <motion.div
            ...
            style={{
              background: `hsl(${artist.themeColor} / 0.45)`,
    ...
    <AnimatePresence mode="wait" custom={direction} initial={false}>
      ...
      >
        ...
        >
          ...
          <MoodBadge moodColor={artist.themeColor} label="Artist" />
...
//      ra ngoài vùng này.
// ─────────────────────────────────────────────────────────────────────────────
const HeroBackground = memo(({ artist }: { artist: IArtist }) => (
  <>
    ...
    >
      ...
      <AnimatePresence>
        <motion.div
          ...
          style={{
            background: `radial-gradient(ellipse 140% 80% at 50% 0%, hsl(${artist.themeColor} / 0.07) 0%, transparent 55%)`,
            willChange: "opacity",
          }}
        />
      </AnimatePresence>
      ...
      <AnimatePresence>
        <motion.div
          ...
          style={{
            background: `radial-gradient(ellipse 65% 55% at 32% 42%, hsl(${artist.themeColor} / 0.1) 0%, transparent 60%)`,
            willChange: "opacity",
          }}
        />
      </AnimatePresence>
...
export function ArtistHero() {
  ...
  // Normalize API data to HeroArtist shape
  ...
  const {
    ...
    isThisArtistPlaying,
    isFetching,
  } = useArtistPlayback(currentArtist);
  ...
  const handleNavigateArtist = useCallback(() => {
    if (!isDragging.current && currentArtist) {
      navigate(`/artists/${currentArtist.slug}`);
    }
  ...
  // Deep Error
  if (isError || !hasResults || !currentArtist) {
    return (
      <>
  ...
  return (
    <section
      ...
      style={{ "--hero-mood": currentArtist.themeColor } as React.CSSProperties}
      aria-label="Featured artists"
    >
      {/* ── BACKGROUND (isolated, no Hero re-render cascade) ── */}
      <HeroBackground artist={currentArtist} />
