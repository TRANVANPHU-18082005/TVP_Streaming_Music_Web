...
const ArtworkCard = memo(
  ({
    ...
    const boxShadow = useTransform(
      [shadowX, shadowY],
      ([sx, sy]) =>
        `${sx} ${sy} 48px hsl(${playlist.themeColor} / 0.35), 0 24px 64px rgba(0,0,0,0.45)`,
    );
      ...
      <AnimatePresence mode="popLayout" custom={direction} initial={false}>
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background: `radial-gradient(circle, hsl(${playlist.themeColor} / 0.7) 0%, transparent 70%)`,
              filter: "blur(55px)",
          ...
          <motion.div
            ...
            style={{
              background: `hsl(${playlist.themeColor} / 0.45)`,
    ...
    <AnimatePresence mode="wait" custom={direction} initial={false}>
      ...
      >
        ...
        >
          ...
          <MoodBadge moodColor={playlist.themeColor} label="Playlist" />
        ...
        >
          <span className="text-muted-foreground">Thuộc</span>
          <button className="font-bold text-foreground transition-all duration-300 relative group/artist hover:text-primary">
            "Tvp Music"
...
//      ra ngoài vùng này.
// ─────────────────────────────────────────────────────────────────────────────
const HeroBackground = memo(({ playlist }: { playlist: IPlaylist }) => (
  <>
    ...
    >
      ...
      <AnimatePresence>
        <motion.div
          key={`bg-tint-light-${playlist._id}`}
          ...
          style={{
            background: `radial-gradient(ellipse 140% 80% at 50% 0%, hsl(${playlist.themeColor} / 0.07) 0%, transparent 55%)`,
            willChange: "opacity",
          }}
        />
      </AnimatePresence>
      ...
      <AnimatePresence>
        <motion.div
          ...
          style={{
            background: `radial-gradient(ellipse 65% 55% at 32% 42%, hsl(${playlist.themeColor} / 0.1) 0%, transparent 60%)`,
            willChange: "opacity",
          }}
        />
      </AnimatePresence>
...
export function PlaylistHero() {
  ...
  // Normalize API data to HeroPlaylist shape
  ...
  const {
    ...
    isFetching,
  } = usePlaylistPlayback(currentPlaylist);
  ...
  const handleNavigatePlaylist = useCallback(() => {
    if (!isDragging.current && currentPlaylist) {
      navigate(`/playlists/${currentPlaylist.slug}`);
    }
  }, [currentPlaylist, navigate]);
  ...
  // Deep Error
  if (isError || !hasResults || !currentPlaylist) {
    return (
      <>
  ...
  return (
    <section
      ...
      style={
        { "--hero-mood": currentPlaylist.themeColor } as React.CSSProperties
      }
      aria-label="Featured playlists"
    >
      {/* ── BACKGROUND (isolated, no Hero re-render cascade) ── */}
      <HeroBackground playlist={currentPlaylist} />
