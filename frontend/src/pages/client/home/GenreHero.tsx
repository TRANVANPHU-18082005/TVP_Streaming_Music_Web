...
import { IGenre, useTrendingGenres } from "@/features/genre";
...
const ArtworkCard = memo(
  ({
    ...
    const boxShadow = useTransform(
      [shadowX, shadowY],
      ([sx, sy]) =>
        `${sx} ${sy} 48px hsl(${genre.color} / 0.35), 0 24px 64px rgba(0,0,0,0.45)`,
    );
      ...
      <AnimatePresence mode="popLayout" custom={direction} initial={false}>
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background: `radial-gradient(circle, hsl(${genre.color} / 0.7) 0%, transparent 70%)`,
              filter: "blur(55px)",
          ...
          <motion.div
            ...
            style={{
              background: `hsl(${genre.color} / 0.45)`,
    ...
    <AnimatePresence mode="wait" custom={direction} initial={false}>
      ...
      >
        ...
        >
          ...
          <MoodBadge moodColor={genre.color} label="Genre" />
  ...
  ({
    ...
    <div className="flex items-center flex-wrap gap-3 pt-1 justify-center lg:justify-start">
      ...
      <motion.button
        ...
        transition={SPRING_SNAPPY}
        aria-label={isPlaying ? "Tạm dừng genre" : "Phát genre"}
...
//      ra ngoài vùng này.
// ─────────────────────────────────────────────────────────────────────────────
const HeroBackground = memo(({ genre }: { genre: IGenre }) => (
  <>
    ...
    >
      ...
      <AnimatePresence>
        <motion.div
          ...
          style={{
            background: `radial-gradient(ellipse 140% 80% at 50% 0%, hsl(${genre.color} / 0.07) 0%, transparent 55%)`,
            willChange: "opacity",
          }}
        />
      </AnimatePresence>
      ...
      <AnimatePresence>
        <motion.div
          ...
          style={{
            background: `radial-gradient(ellipse 65% 55% at 32% 42%, hsl(${genre.color} / 0.1) 0%, transparent 60%)`,
            willChange: "opacity",
          }}
        />
      </AnimatePresence>
...
export function GenreHero() {
  ...
  const handleNavigateGenre = useCallback(() => {
    if (!isDragging.current && currentGenre) {
      navigate(`/genres/${currentGenre.slug}`);
    }
  ...
  // Deep Error
  if (isError || !hasResults || !currentGenre) {
    return (
      <>
  ...
  return (
    <section
      ...
      style={{ "--hero-mood": currentGenre.color } as React.CSSProperties}
      aria-label="Featured genres"
    >
      {/* ── BACKGROUND (isolated, no Hero re-render cascade) ── */}
      <HeroBackground genre={currentGenre} />
              ...
              >
                ...
                >
                  <ArtworkCard
                    ...
                    onNavigate={handleNavigateGenre}
