...
import { ITrack, useFeaturedTracks } from "@/features/track";
...
const ArtworkCard = memo(
  ({
    ...
    const boxShadow = useTransform(
      [shadowX, shadowY],
      ([sx, sy]) =>
        `${sx} ${sy} 48px hsl(--primary / 0.35), 0 24px 64px rgba(0,0,0,0.45)`,
    );
      ...
      <AnimatePresence mode="popLayout" custom={direction} initial={false}>
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background: `radial-gradient(circle, hsl(--primary / 0.7) 0%, transparent 70%)`,
              filter: "blur(55px)",
    ...
    <AnimatePresence mode="wait" custom={direction} initial={false}>
      ...
      >
        ...
        >
          ...
          <MoodBadge moodColor="hsl(--primary)" label="Track" />
...
//      ra ngoài vùng này.
// ─────────────────────────────────────────────────────────────────────────────
const HeroBackground = memo(({ track }: { track: ITrack }) => (
  <>
    ...
    >
      ...
      <AnimatePresence>
        <motion.div
          ...
          style={{
            background: `radial-gradient(ellipse 140% 80% at 50% 0%, hsl(--primary / 0.07) 0%, transparent 55%)`,
            willChange: "opacity",
          }}
        />
      </AnimatePresence>
      ...
      <AnimatePresence>
        <motion.div
          ...
          style={{
            background: `radial-gradient(ellipse 65% 55% at 32% 42%, hsl(--primary / 0.1) 0%, transparent 60%)`,
            willChange: "opacity",
          }}
        />
      </AnimatePresence>
...
export function TrackHero() {
  ...
  const isThisTrackActive = currentTrackId === tracks[currentIndex]?._id;
  ...
  const handleNavigateTrack = useCallback(() => {
    if (!isDragging.current && currentTrack) {
      navigate(`/tracks/${currentTrack._id}`);
    }
  }, [currentTrack, navigate]);

  const handleNavigateArtist = useCallback(() => {
    if (currentTrack && currentTrack?.artist) {
      navigate(`/artists/${currentTrack.artist.slug}`);
    }
  }, [currentTrack, navigate]);
  ...
  // Deep Error
  if (isError || !hasResults || !currentTrack) {
    return (
      <>
  ...
  return (
    <section
      ...
      style={{ "--hero-mood": "hsl(--primary)" } as React.CSSProperties}
      aria-label="Featured tracks"
    >
      {/* ── BACKGROUND (isolated, no Hero re-render cascade) ── */}
      <HeroBackground track={currentTrack} />
              ...
              >
                ...
                >
                  <ArtworkCard
                    ...
                    onPlay={() => handlePlayTrack(currentTrack)}
                  />
                </motion.div>
              </div>
