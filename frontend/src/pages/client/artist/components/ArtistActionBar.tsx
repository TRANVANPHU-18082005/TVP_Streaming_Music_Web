...
export const ArtistActionBar = memo<ArtistActionBarProps>(
  ({
    artist,
    handleMoreOptions,
    palette,
    ...
    const playSz = isCompact ? "size-10" : "size-14 sm:size-16";
    const playIconSz = isCompact ? "size-4" : "size-6 sm:size-7";
    const ctrlSz = isCompact ? "size-8" : "size-10 sm:size-11";
    ...
    const handleMore = useCallback(
      (e: React.MouseEvent) => {
        e.stopPropagation();
        handleMoreOptions(artist);
      },
      [artist, handleMoreOptions],
    );
    return (
      <div
