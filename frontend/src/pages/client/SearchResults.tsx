...
import { IAlbum, IArtist, IGenre, IPlaylist, ITrack, SearchAlbum, SearchArtist, SearchGenre, SearchPlaylist, TopResultItem } from "@/features";
...
const SearchArtistCard = React.memo(({ artist, onClick }: SearchArtistCardProps) => (
  <motion.div
    onClick={onClick}
    variants={fadeUp}
    whileHover={{ y: -4 }}
  ...
  >
    <div className="relative aspect-square rounded-full overflow-hidden shadow-md dark:bg-white/5 bg-black/5">
      <ImageWithFallback src={toCDN(artist.avatar)} alt={artist.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
    </div>
    <div className="flex flex-col items-center text-center">
      <h3 className="text-[14px] font-bold truncate w-full dark:text-white/90 text-gray-900 group-hover:text-primary transition-colors" dangerouslySetInnerHTML={{ __html: artist.highlightHtml || artist.name }} />
      <p className="text-[12px] dark:text-white/40 text-gray-500 mt-0.5">Nghệ sĩ</p>
    </div>
  </motion.div>
));
...
const SearchAlbumCard = React.memo(({ album, onClick }: SearchAlbumCardProps) => (
  <motion.div
    onClick={onClick}
    variants={fadeUp}
    whileHover={{ y: -4 }}
  ...
  >
    <div className="relative aspect-square rounded-xl overflow-hidden shadow-md dark:bg-white/5 bg-black/5">
      <ImageWithFallback src={toCDN(album.coverImage)} alt={album.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
    </div>
    <div className="flex flex-col">
      <h3 className="text-[14px] font-bold truncate w-full dark:text-white/90 text-gray-900 group-hover:text-primary transition-colors" dangerouslySetInnerHTML={{ __html: album.highlightHtml || album.title }} />
      <p className="text-[12px] dark:text-white/40 text-gray-500 mt-0.5 truncate">{album.artist?.name || "Đĩa nhạc"}</p>
    </div>
  </motion.div>
));
...
const SearchPlaylistCard = React.memo(({ playlist, onClick }: SearchPlaylistCardProps) => (
  <motion.div
    onClick={onClick}
    variants={fadeUp}
    whileHover={{ y: -4 }}
  ...
  >
    <div className="relative aspect-square rounded-xl overflow-hidden shadow-md dark:bg-white/5 bg-black/5">
      <ImageWithFallback src={toCDN(playlist.coverImage)} alt={playlist.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
    </div>
    <div className="flex flex-col">
      <h3 className="text-[14px] font-bold truncate w-full dark:text-white/90 text-gray-900 group-hover:text-primary transition-colors" dangerouslySetInnerHTML={{ __html: playlist.highlightHtml || playlist.title }} />
      <p className="text-[12px] dark:text-white/40 text-gray-500 mt-0.5 truncate">Danh sách phát</p>
    </div>
  </motion.div>
));
...
const SearchGenreCard = React.memo(({ genre, onClick }: SearchGenreCardProps) => (
  <motion.div
    onClick={onClick}
    variants={fadeUp}
  ...
  >
    ...
    <h3 className="absolute top-4 left-4 text-white text-lg font-black tracking-tight" dangerouslySetInnerHTML={{ __html: genre.highlightHtml || genre.name }} />
    <ImageWithFallback src={toCDN(genre.image)} alt={genre.name} className="absolute -bottom-2 -right-4 w-24 h-24 object-cover rounded shadow-xl rotate-[25deg] transition-transform duration-500 group-hover:rotate-[20deg] group-hover:scale-110" />
  </motion.div>
));
    ...
    >
      ...
      <div className="w-6 shrink-0 flex justify-center items-center">
        {isCurrentPlaying ? (
          ...
          <span
            className={cn("text-xs font-mono dark:text-white/28 text-gray-400")}
          >
            {index + 1}
          </span>
        )}
      ...
      <div className="relative size-11 shrink-0 rounded-lg overflow-hidden dark:bg-white/8 bg-black/6 shadow-md" onClick={onPlay}>
        <ImageWithFallback
...
const TopResultCard = React.memo(({ item, isCurrentPlaying, isLoadingThis, onPlay }: TopResultCardProps) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <ImageWithFallback
            src={type === "artist" ? item.avatar : type === "genre" ? item.image : item.coverImage}
            alt={type === "artist" ? item.name : type === "genre" ? item.name : item.title}
        ...
        <div className="relative z-10 mt-28 flex flex-col gap-2">
          <h3 className="text-2xl font-black tracking-tight dark:text-white text-gray-900 line-clamp-2 group-hover:text-primary transition-colors"
            dangerouslySetInnerHTML={{ __html: item.highlightHtml || (type !== "artist" && type !== "genre" ? item.title : item.name) }}
          />
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black uppercase tracking-[0.15em] px-2.5 py-1 rounded-full dark:bg-white/8 bg-black/6 dark:text-white/55 text-gray-600">
              {type === "artist" ? "Nghệ sĩ" : type === "album" ? "Album" : type === "playlist" ? "Playlist" : type === "genre" ? "Thể loại" : "Bài hát"}
            </span>
            {type !== "artist" && type !== "genre" && type !== "playlist" && item.artist?.name && (
        ...
        >
          {activeTab === "all" && searchResult?.topResult && (
            <TopResultCard
              ...
              isCurrentPlaying={
                isGlobalPlaying && currentTrackId === searchResult.topResult._id
              }
          ...
          {searchResult?.tracks?.length > 0 && (
            <div className="flex flex-col gap-1">
              <SectionHeader
                title="Bài hát"
                showMore={activeTab === "all" && searchResult.tracks.length > 4}
              ...
              >
                {(activeTab === "all"
                  ...
                  <TrackRow
                    ...
                    onPlay={(e: any) => handlePlayTrack(e, track)}
      ...
      {(activeTab === "all" || activeTab === "artist") &&
        searchResult?.artists?.length > 0 && (
          <div>
            <SectionHeader
              title="Nghệ sĩ"
              showMore={activeTab === "all" && searchResult.artists.length > 6}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {(activeTab === "all"
                ...
                <SearchArtistCard key={artist._id} artist={artist} onClick={() => handleResultClick(`/artists/${artist.slug}`)} />
              ))}
            </div>
          </div>
        )}
      ...
      {(activeTab === "all" || activeTab === "album") &&
        searchResult?.albums?.length > 0 && (
          <div>
            <SectionHeader
              title="Đĩa nhạc"
              showMore={activeTab === "all" && searchResult.albums.length > 5}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5">
              {(activeTab === "all"
                ...
                <SearchAlbumCard key={album._id} album={album} onClick={() => handleResultClick(`/albums/${album.slug}`)} />
              ))}
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5">
              {(activeTab === "all"
                ...
                <SearchPlaylistCard key={pl._id} playlist={pl} onClick={() => handleResultClick(`/playlists/${pl._id}`)} />
              ))}
            </div>
          </div>
        )}

      {(activeTab === "all" || activeTab === "genre") &&
        searchResult?.genres?.length > 0 && (
          <div>
            <SectionHeader
              title="Thể loại"
              showMore={
                activeTab === "all" && searchResult.genres.length > 5
              }
              onMore={() => switchTab("genre")}
            />
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5">
              {(activeTab === "all"
                ? searchResult.genres.slice(0, 5)
                : searchResult.genres
              ).map((genre: any) => (
                <SearchGenreCard key={genre._id} genre={genre} onClick={() => handleResultClick(`/genres/${genre.slug}`)} />
              ))}
            </div>
          </div>
        )}
