...
const AlbumCard: React.FC<AlbumCardProps> = ({
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-start gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40 rounded-xl">
                ...
                {album.isDeleted ? (
                  <DropdownMenuItem
                    onClick={(e) => handleAction(e, () => setDeleteAlbum(true))}
                    className="font-medium cursor-pointer text-success"
                  >
                    <RotateCcw className="mr-2 size-4" /> Restore album
                  </DropdownMenuItem>
                ) : (
                  <DropdownMenuItem
                    onClick={(e) => handleAction(e, () => setDeleteAlbum(true))}
      ...
      <ConfirmationModal
        isOpen={!!deleteAlbum}
        onCancel={() => setDeleteAlbum(false)}
        onConfirm={onDelete}
        title={album.isDeleted ? "Khôi phục album" : "Xóa album?"}
        variant={album.isDeleted ? "info" : "destructive"}
        isLoading={isMutating}
        countdownSeconds={3}
        description={
          <div className="space-y-4">
            <p className="text-sm text-foreground/80">
              Bạn có chắc chắn muốn {album.isDeleted ? "Khôi phục" : "xóa"}{" "}
              {
        ...
        confirmLabel={album.isDeleted ? "Khôi phục" : "Xóa"}
        isDestructive
      />
    </>
  );
