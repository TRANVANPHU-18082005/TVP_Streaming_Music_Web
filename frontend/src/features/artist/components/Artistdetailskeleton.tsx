...
/**
 * @file ArtistDetailSkeleton.tsx — Full-page + embedded skeleton for ArtistDetailPage
 *
 * Design System: Soundwave (Obsidian Luxury / Neural Audio)
 *
 * ARCHITECTURE
 *   ArtistDetailSkeleton           — smart wrapper, accepts variant prop
 *   ├── PageVariantSkeleton        — full-page layout
 *   │   ├── HeroSkeleton           — immersive hero (460–620px tall)
 *   │   │   ├── BackNavSkeleton    — chevron + "Quay lại"
 *   │   │   ├── AvatarSkeleton     — 160–260px circle, multi-border
 *   │   │   └── HeroInfoSkeleton   — verified badge + name + stats pills + genres
 *   │   ├── ActionBarSkeleton      — play + shuffle + follow pill + more
 *   │   └── BodySkeleton           — lg:grid-cols-12 layout
 *   │       ├── LeftColumnSkeleton — tracks + gallery strip + album grid
 *   │       └── RightColumnSkeleton— bio card + social links
 *   └── EmbeddedVariantSkeleton    — compact scrollable panel
 *       ├── EmbeddedHeaderSkeleton — small avatar + info
 *       ├── ActionBarSkeleton      — reused, compact
 *       ├── TrackListSkeleton      — 5 rows
 *       ├── AlbumGridSkeleton      — 2-col, 4 cards
 *       └── BioSnippetSkeleton     — rounded card
 *
 * EXACT DIMENSION MATCH → zero CLS:
 *   Avatar page:    size-[160px] sm:size-[210px] md:size-[260px], rounded-full
 *   Avatar embedded: size-20, rounded-2xl
 *   Hero height:    min-h-[460px] sm:min-h-[520px] md:min-h-[620px]
 *   Track row:      ≈ 48px (py-2.5 + content)
 *   Gallery card:   aspect-[16/10] w-[82vw] sm:w-[360px] md:w-[440px]
 *   Album card:     aspect-square, rounded-[18px]
 *
 * index.css TOKENS USED
 *   .skeleton            §15  shimmer animation
 *   .skeleton-text       §15  h-3.5 + radius-pill
 *   .skeleton-cover      §15  radius-xl + aspect:1
 *   .scrollbar-thin      § 5  thin scrollbar
 *   .no-scrollbar        § 5  hide scrollbar
 *   .glass-frosted       § 6  T2 glassmorphism (bio card)
