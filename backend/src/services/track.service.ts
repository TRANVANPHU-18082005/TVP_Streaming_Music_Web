...
class TrackService {
  ...
  // ── 8. TOP HOT TODAY (PAGED) ────────────────────────────────────────────
  async getTopHotTracksToday(filter: any) {
    const { page = 1, limit = 20 } = filter as any;
    const skip = (Number(page) - 1) * Number(limit);
    const totalNeeded = skip + Number(limit);

    try {
      const start = new Date();
      start.setHours(0, 0, 0, 0);

      // 1. Lấy danh sách ID từ PlayLog (Realtime)
      const hotTrackLogs = await PlayLog.aggregate([
        { $match: { listenedAt: { $gte: start } } },
        { $project: { trackId: 1, listener: { $ifNull: ["$userId", "$ip"] } } },
        { $group: { _id: { trackId: "$trackId", listener: "$listener" } } },
        { $group: { _id: "$_id.trackId", score: { $sum: 1 } } },
        { $sort: { score: -1 } },
        { $limit: 1000 }, // Giới hạn pool để hiệu năng tốt
      ]);

      let hotIds = hotTrackLogs.map((t) => t._id.toString());

      // 2. Fallback: Nếu tổng số ID realtime chưa đủ lấp đầy trang yêu cầu
      const totalValidTracks = await Track.countDocuments({
        isDeleted: { $ne: true },
        isPublic: true,
        status: "ready",
      });

      if (hotIds.length < totalNeeded && hotIds.length < totalValidTracks) {
        const neededMore = totalNeeded - hotIds.length;
        const extraTracks = await Track.find({
          _id: { $nin: hotIds },
          isDeleted: { $ne: true },
          isPublic: true,
          status: "ready",
        })
          .sort({ playCount: -1 }) // Bù bằng bài có lượt nghe cao nhất mọi thời đại
          .limit(neededMore)
          .select("_id")
          .lean();

        hotIds = [...hotIds, ...extraTracks.map((t) => t._id.toString())];
      }

      // 3. Phân trang trên mảng ID
      const pageIds = hotIds.slice(skip, skip + Number(limit));

      // 4. Hydrate (Lấy data chi tiết)
      const data = await Track.find({ _id: { $in: pageIds } })
        .populate(TRACK_POPULATE as any)
        .select(TRACK_SELECT)
        .lean();

      // Sắp xếp lại data theo đúng thứ tự của pageIds
      const sortedData = pageIds
        .map((id) => data.find((t) => t._id.toString() === id))
        .filter(Boolean);

      return {
        data: sortedData,
        meta: {
          totalItems: Math.max(hotTrackLogs.length, totalValidTracks),
          page: Number(page),
          pageSize: Number(limit),
          totalPages: Math.ceil(
            Math.max(hotTrackLogs.length, totalValidTracks) / Number(limit),
          ),
          hasNextPage:
            totalNeeded < Math.max(hotTrackLogs.length, totalValidTracks),
        },
      };
    } catch (error) {
      console.error("Error in getTopHotTracksToday:", error);
      throw error;
    }
  }

  // ── 9. TOP FAVOURITE TRACKS (PAGED) ──────────────────────────────────────
  async getTopFavouriteTracks(filter: any) {
    ...
    try {
      ...
      let favIds = favLogs.map((l) => l._id.toString());

      // 2. Fallback: Nếu không đủ, lấy bài có likeCount cao nhất trong bảng Track
      const totalValidTracks = await Track.countDocuments({
        isDeleted: { $ne: true },
        isPublic: true,
        status: "ready",
      ...
      if (favIds.length < totalNeeded && favIds.length < totalValidTracks) {
        const neededMore = totalNeeded - favIds.length;
        const extraTracks = await Track.find({
          _id: { $nin: favIds },
          isDeleted: { $ne: true },
          isPublic: true,
        ...
        favIds = [...favIds, ...extraTracks.map((t) => t._id.toString())];
      }

      const pageIds = favIds.slice(skip, skip + Number(limit));
      ...
      return {
        data: sortedData,
        meta: {
          totalItems: Math.max(favLogs.length, totalValidTracks),
          page: Number(page),
          pageSize: Number(limit),
          totalPages: Math.ceil(
            Math.max(favLogs.length, totalValidTracks) / Number(limit),
          ),
          hasNextPage: totalNeeded < Math.max(favLogs.length, totalValidTracks),
        },
      };
    } catch (error) {
      console.error("Error in getTopFavouriteTracks:", error);
      // Trả về trang trống thay vì crash app nếu lỗi nặng
      return {
        data: [],
        meta: {
          totalItems: 0,
